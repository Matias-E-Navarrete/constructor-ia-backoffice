...
export default function Habits() {
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-xl font-semibold text-slate-900 mb-4">Hábitos</h1>
      ...
      <ul className="space-y-2">
        {habits.map((h) => (
          <li key={h.id} className="bg-white border border-slate-200 rounded-lg p-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => handleShowStats(h.id)} className="text-sm bg-slate-100 text-slate-700 px-3 py-1 rounded-md">
                  Estadísticas
                </button>
              </div>
            </div>
        ...
        {habits.length === 0 && <p className="text-slate-500 text-sm">Todavía no agregaste ningún hábito.</p>}
      </ul>
    </div>
  );
}
